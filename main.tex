...
Due to the large amount of data created by second- and third-generation sequencing (also known as next generation and long-read sequencing, respectively), data analysis is a complex, difficult to reproduce and computationally intensive process. Hence, sequence data must be processed by refined workflows which can requisite bioinformatics skills \cite{Hemlata2016}. Each step of the analysis may require a set of different tools or software. For example, de novo assembly is done through alignment, assembly and polishing steps which requires the use of multiple tools, with their own parameters. The alignment step can be done by Minimap2, designed for pairwise alignment, but does not conclude the de novo assembly, therefore miniasm and racon are also deployed. This makes it more complex, because the tools are executed from a command-line and require extensive computational resources. The Galaxy platform implements a user-friendly interface to accommodate these command-line tools with their dependencies and refined workflows. This empowers researchers to do powerful analysis, including assembly of second- or third-generation sequence data, without the need for programming experience. Galaxy offers a wide range of tools for over 125 subjects. It has over 7000 citations, and with more than 7200 tools it is widely used in the biological science community \cite{Galaxycitations, Galaxytoolshed}. NanoGalaxy integrates the needed third-generation sequencing tools for a user-friendly, but powerful platform for ultimately on-site analysis for a variety of analysis subjects demanded by the biological science community.
...
The long read assembly workflow is a fast way to assemble genomes but lacks the ability to scan for single nucleotide polymorphisms (SNPs). This is because third-generation sequencing has a higher error rate. Therefore, a workflow processing second- and third-generation sequences is included to combine the best features of both sequencing methods. The workflow recommended by the Unicycler developers \cite{Wick2017} includes Trim Galore!, Porechop and Filtlong for quality trimming, Unicycler for de novo assembly and bandage for plasmid visualization. These tools are made available in NanoGalaxy as stand-alone tools and combined in a workflow. The Assembly graphs are compared to the results from \citet{Wick2017}. The Illumina-only (second-generation sequencing) graphs show an unclear visualization of plasmids, where nanopore-only (third generation sequencing) is able to achieve the structure of plasmids. The combination of both sequence techniques gives the clearest view of plasmids and shows the same result as shown by \citet{Wick2017} (Figure \ref{figure:WickEtAl}).
...
The implementation of the tools and workflows in Galaxy gives a non-programmer the ability to do extensive research on third-generation sequence data without coding. All tools and their dependencies are installed through the Galaxy platform and are managed by the Conda framework for dependency management. The tools and their dependencies are available from the Bioconda Conda channel \cite{Gruning2018}.
